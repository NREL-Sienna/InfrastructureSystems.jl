...
function get_static_time_series_summary_table(store::TimeSeriesMetadataStore)
    category_clause = _make_category_clause(StaticTimeSeries)
    query = """
        SELECT
            owner_type
            ,owner_category
...
function get_forecast_summary_table(store::TimeSeriesMetadataStore)
    category_clause = _make_category_clause(Forecast)
    query = """
        SELECT
            owner_type
            ,owner_category
...
function _make_category_clause(ts_type::Type{<:TimeSeriesData})
    ...
    return if length(subtypes) == 1
        "time_series_type = '$(subtypes[1])'"
    else
        types = join(("'$x'" for x in subtypes), ",")
        "time_series_type IN ($types)"
    end
end
