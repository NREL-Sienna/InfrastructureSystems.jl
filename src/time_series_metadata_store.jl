...
function _create_indexes!(store::TimeSeriesMetadataStore)
    ...
    # 2. Optimize for checks at system.add_time_series. Use all fields and features hash.
    ...
    SQLite.createindex!(
        ...
        METADATA_TABLE_NAME,
        "by_c_n_tst_hash",
        ["owner_uuid", "name", "time_series_type", "features_hash"];
        unique = false,
    )
    SQLite.createindex!(
        store.db,
...
function add_metadata!(
    ...
    feature_hash = _compute_feature_hash(metadata.features)
    vals = _create_row(
        metadata,
        owner,
        owner_category,
...
function has_metadata(
    ...
    attrs = Dict{Symbol, Any}(Symbol(k) => v for (k, v) in (get_features(metadata)))
    return has_metadata(
        store,
        owner,
...
function has_time_series(
    ...
    where_clause =
        _make_where_clause(owner; time_series_type = time_series_type, name = name)
    return _has_time_series(store, where_clause)
end
...
function _compute_feature_hash(features::Dict{String, <:Any})
    attrs = Dict(Symbol(k) => v for (k, v) in features)
    return _compute_feature_hash(; attrs...)
end

function _compute_feature_hash(; features...)
    isempty(features) && return nothing
    attrs = _make_feature_dict(; features...)
    return _compute_hash(JSON3.write(attrs))
end

_compute_hash(text::String) = bytes2hex(SHA.sha256(text))
...
function _make_feature_filter(; features...)
    attrs = _make_feature_dict(; features...)
    text =
        join((["metadata->>'\$.features.$k' = '$v'" for (k, v) in attrs]), "AND ")
    return "($text)"
end

function _make_feature_hash_filter(feature_hash::String)
    return "features_hash = '$feature_hash'"
end

function _make_feature_dict(; features...)
    key_names = sort!(collect(string.(keys(features))))
    return OrderedDict{String, Any}(k => features[Symbol(k)] for k in key_names)
end
...
function _make_where_clause(
    ...
    time_series_type::Union{Type{<:TimeSeriesData}, Nothing} = nothing,
    name::Union{String, Nothing} = nothing,
    feature_hash::Union{String, Nothing} = nothing,
    features...,
)
    vals = String[]
    if !isnothing(owner)
    ...
    if isnothing(feature_hash) && !isempty(features)
        push!(vals, "$(_make_feature_filter(; features...))")
    end
    if !isnothing(feature_hash)
        push!(vals, "$(_make_feature_hash_filter(feature_hash))")
    end
...
function _make_where_clause(
    ...
    return _make_where_clause(
        ...
        feature_hash = _compute_feature_hash(get_features(metadata)),
    )
end
