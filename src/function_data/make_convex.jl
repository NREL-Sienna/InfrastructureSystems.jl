...
# ============================================================================
# DECREASING CURVE CONCAVE APPROXIMATION
# ============================================================================

"""
    decreasing_curve_convex_approximation(curve::ValueCurve; kwargs...) -> Union{ValueCurve, Nothing}

Transform a strictly decreasing `ValueCurve` into a concave form, with data quality validation.

This function first validates that the curve data is reasonable and physically meaningful
using [`is_valid_data`](@ref). It also checks that the curve is strictly decreasing using
[`is_strictly_decreasing`](@ref). If either check fails, the function logs an error and
returns `nothing` to trigger a fallback path.

If the data passes validation and is already concave, returns the original curve
(optionally with colinear segments merged).

If the data passes validation but is non-concave, applies antitonic regression to
produce a concave approximation.

# Supported curve types
- `InputOutputCurve{PiecewiseLinearData}`: Antitonic regression on slopes
- `IncrementalCurve{PiecewiseStepData}`: Converts to IO curve, makes concave, converts back
- `AverageRateCurve{PiecewiseStepData}`: Converts to IO curve, makes concave, converts back

Note 1: `IncrementalCurve{LinearFunctionData}` and `AverageRateCurve{LinearFunctionData}` are
intentionally NOT supported. These represent derivatives of quadratic functions. The arbitrary projection approach used for quadratics is not appropriate.
Note 2: `InputOutputCurve{LinearFunctionData}` is not supported because it never presents concavity issues.
Note 3: `InputOutputCurve{QuadraticFunctionData}` is not supported given that it represents a significant change on the curve

# Keyword Arguments
- `weights::Symbol`: Weighting scheme for antitonic regression (affects how violations are resolved)
  - `:length` (default): weight segments by x-extent (wider segments have more influence)
  - `:uniform`: all segments equally weighted
- `anchor`: Point preservation strategy for reconstructing points from new slopes
  - `:first` (default): preserve first point, propagate forward
  - `:last`: preserve last point, propagate backward
  - `:centroid`: minimize total vertical displacement
- `merge_colinear`: Whether to merge colinear segments before concavification (default: `true`)
  - Colinear segments are those where consecutive slopes differ by less than a tolerance
  - This cleanup step removes artificial segmentation that can cause false non-concave detections

# Returns
- The concave curve (original or approximation) if successful
- `nothing` if data quality validation fails or curve is not strictly decreasing

# See also
- [`increasing_curve_convex_approximation`](@ref): for strictly increasing curves
"""
function decreasing_curve_convex_approximation end
