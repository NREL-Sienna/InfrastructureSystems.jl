...
@testset "Test OptimizationProblemResults long format" begin
    ...
    @test @rsubset(var_res, :component == "c1")[!, :value] == [1.0, 2.0]
    @test @rsubset(var_res, :component == "c2")[!, :value] == [3.0, 4.0]
    IS.Optimization.convert_result_to_natural_units(::Type{<:MockVariable}) = true
    var_res = read_variable(opt_res1, var_key)
    @test @rsubset(var_res, :component == "c1")[!, :value] == [10.0, 20.0]
    @test @rsubset(var_res, :component == "c2")[!, :value] == [30.0, 40.0]
    ...
    @test @rsubset(exp_res, :component == "c1")[!, :value] == [1.0, 2.0]
    @test @rsubset(exp_res, :component == "c2")[!, :value] == [3.0, 4.0]
    IS.Optimization.convert_result_to_natural_units(::Type{<:MockExpression}) = true
    exp_res = read_expression(opt_res2, exp_key)
    @test @rsubset(exp_res, :component == "c1")[!, :value] == [10.0, 20.0]
    @test @rsubset(exp_res, :component == "c2")[!, :value] == [30.0, 40.0]
