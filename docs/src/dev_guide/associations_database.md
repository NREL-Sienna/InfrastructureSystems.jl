...
df = InfrastructureSystems.sql(store, "SELECT * FROM time_series_associations WHERE owner_type = 'Generator'")

# Query supplemental attribute associations  
df = InfrastructureSystems.sql(associations, "SELECT * FROM supplemental_attributes WHERE component_type = 'Bus'")
```
